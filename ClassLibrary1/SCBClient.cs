...
namespace SCB.Client
{
    public class SCBClient
    {
        ...
        private List<SCBNode> GetChildNodes(string url)
        {
        ...
        public SCBMetaData GetMetaData(string url)
        {
            var rp = GetRequest(url).Result;
            SCBMetaData table = JsonConvert.DeserializeObject<SCBMetaData>(rp);
            return table;
        }

        private static async Task<string> PostRequest(string url, string jsonQueryString)
        {
            var resp = await client.PostAsync(url, new StringContent(jsonQueryString, Encoding.UTF8, "application/json"));
        ...
        public SCBTable GetTable(SCBNode parent, SCBQuery query)
        {
        ...
        public List<SCBNode> GetNodesBelow(SCBNode parent)
        {
            List<SCBNode> nodes = new List<SCBNode>();
            if (parent.type.Equals("l"))
            {
                nodes = GetChildNodes(parent.path);
            }
            return nodes;
        }
